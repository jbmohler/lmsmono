...
-- Grant usage, DML, and sequence access on all application schemas
-- (schemas are created by the dump import; this covers a fresh DB too)
DO \$\$
DECLARE
...
BEGIN
  FOREACH s IN ARRAY schemas LOOP
    IF EXISTS (SELECT FROM information_schema.schemata WHERE schema_name = s) THEN
      EXECUTE format('GRANT USAGE ON SCHEMA %I TO $DB_APP_USER', s);
      EXECUTE format('GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA %I TO $DB_APP_USER', s);
      EXECUTE format('GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA %I TO $DB_APP_USER', s);
    END IF;
  END LOOP;
END
\$\$;
...
ALTER DEFAULT PRIVILEGES FOR ROLE $DB_OWNER IN SCHEMA contacts
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO $DB_APP_USER;
ALTER DEFAULT PRIVILEGES FOR ROLE $DB_OWNER IN SCHEMA contacts
  GRANT USAGE, SELECT ON SEQUENCES TO $DB_APP_USER;

ALTER DEFAULT PRIVILEGES FOR ROLE $DB_OWNER IN SCHEMA hacc
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO $DB_APP_USER;
ALTER DEFAULT PRIVILEGES FOR ROLE $DB_OWNER IN SCHEMA hacc
  GRANT USAGE, SELECT ON SEQUENCES TO $DB_APP_USER;

ALTER DEFAULT PRIVILEGES FOR ROLE $DB_OWNER IN SCHEMA yenotsys
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO $DB_APP_USER;
ALTER DEFAULT PRIVILEGES FOR ROLE $DB_OWNER IN SCHEMA yenotsys
  GRANT USAGE, SELECT ON SEQUENCES TO $DB_APP_USER;
SQL
